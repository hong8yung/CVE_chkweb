...
- 검토 상태는 백로그(`daily_review_backlog`) 기준으로 CVE 단위 누적 관리
- `reviewed/ignored` 상태 CVE라도 `cve.last_modified_at`이 이후 갱신되면 `재검토 필요`로 표시
...
- 마우스로 행을 한 번 선택한 뒤 `↑/↓`로 이전/다음 행으로 이동 가능 (이동만 수행)
- `Space` 키로 현재 활성 행의 체크박스 토글 가능
- `Shift + ↑/↓`로 앵커 행부터 현재 행까지 범위 선택 가능
- 체크박스가 선택된 행만 배경 음영 표시, 키보드 활성 행은 테두리/글로우로 별도 강조
- 우측 드로어가 열린 상태에서 `↑/↓` 이동 시 활성 행의 Description으로 자동 동기화
...
- 입력 중 `vendor/product/version` 3열 미리보기 API (`/api/cpe/preview`) 제공
- 현재 입력값을 프리셋으로 저장하고, 프리셋 활성/비활성/삭제 가능
- 프리셋 이름 변경/복제/수정시각 확인 가능
